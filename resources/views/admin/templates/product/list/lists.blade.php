    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="card-footer text-sm sticky-top">
                <a class="btn btn-sm bg-gradient-primary text-white" href="{{ route('addlist') }}" title="Thêm mới"><i
                        class="fas fa-plus mr-2"></i>Thêm mới</a>
                <a class="btn btn-sm bg-gradient-danger text-white" id="delete-all"
                    data-href="{{route('deletelist',['page'=>$page])}}" title="Xóa tất cả" onclick="deleteListItems()"><i
            ...
            <div class="card card-primary card-outline text-sm mb-0">
                <div class="row card-header">
                    <div class="col-4">
                        <h3 class="card-title">Danh sách Sản phẩm cấp 1</h3>
                    </div>
                    @if (session('success'))
                        <div class="col-8">
                                ...
                                @foreach ($items as $k => $v)
                                    <tr>
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox{{ $v->id }}"
                                                    id="show-checkbox-noibat-{{ $v->id }}" data-table="product_list"
                                                    data-href="{{route('ajaxlistStatus')}}"
                                                    data-id="{{ $v->id }}" data-loai="noibat" onclick="changeStatus({{ $v->id }},'show-checkbox')"
                                                    {{ $v->noibat == 1 ? 'checked' : '' }}>
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox-{{ $v->id }}"
                                                    ...
                                                    data-href="{{route('ajaxlistStatus')}}"
                                                    data-id="{{ $v->id }}" data-loai="hienthi" onclick="changeStatus({{ $v->id }},'show-checkbox-')"
                                                    {{ $v->hienthi == 1 ? 'checked' : '' }}>
                                        ...
                                        <td class="align-middle text-center text-md text-nowrap">
                                            <a class="text-primary mr-2" href="{{ route('editlist',['id' => $v->id,'page'=>$page]) }}"
                                                title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                                            <a class="text-danger" id="delete-item-{{ $v->id }}"
                                                data-href="{{ route('deletelist', ['id' => $v->id,'page'=>$page]) }}"
